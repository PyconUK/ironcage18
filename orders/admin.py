...
class OrderRowInline(OurActionsOnlyMixin, admin.TabularInline):
    ...
    fields = readonly_fields = ('link_to_item', 'item_descr',
                                'item_descr_extra', 'row_cost_inc_vat')
...
class OrderAdmin(DjangoObjectActions, OurActionsOnlyMixin, admin.ModelAdmin):
    ...
    view_on_site = False
    fields = ('link_to_purchaser', 'invoice_number', 'status',
              'stripe_charge_id', 'stripe_charge_created',
              'stripe_charge_failure_reason', 'billing_name', 'billing_addr')
    readonly_fields = fields

    list_display = ('order_id', 'link_to_purchaser', 'purchaser_email',
